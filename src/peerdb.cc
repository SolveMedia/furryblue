...
peerdb_init(void){
    ...
    start_thread(peerdb_periodic, 0, 0);
}
...
PeerDB::report(NTD *ntd){
    ...
    out << "# name                      status    cpu       disk upd          uptime\n";
...
PeerDB::cleanup(void){
    ...
    for(list<Peer*>::iterator it=_allpeers.begin(); it != _allpeers.end(); it++){
        ...
        if( p->_gstatus->timesent() < now - KEEPLOST || p->_gstatus->lastup() < now - KEEPDOWN ){
...
void
PeerDB::reply_peers(ACPY2StatusReply *res){

    _lock.r_lock();

    for(list<Peer*>::const_iterator it=_allpeers.begin(); it != _allpeers.end(); it++){
        const Peer *p = *it;
        ACPY2Status *g = res->add_status();
        p->status_reply( g );
    }
...
#define MAXLOAD 999999
    ...
    return peer->_gstatus->cpu_metric();
#undef MAXLOAD
}
...
_update_ok(ACPY2Status *g){
    ...
    if( g->subsystem().compare(MYNAME) )                      return 0; // and subsystem?
    if( g->timesent()  < lr_now() - KEEPLOST  )   	      return 0; // we'd just toss it out
...
void
PeerDB::add_peer(ACPY2Status *g){

    const string *id = & g->server_id();

    if( !_update_ok(g) ){
        DEBUG("add peer/no %s", id->c_str());
        return;
    }
...
void
PeerDB::add_sceptical(ACPY2Status *g){
